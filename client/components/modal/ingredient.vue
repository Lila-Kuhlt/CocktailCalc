<template>
  <Modal @submit="upsert_ingredient" ref="modal">
    <form
    ...
    >
      <div>
        ...
        <FormInput class="w-full" type="text" name="name" required />
      </div>

      <div>
        <div class="font-semibold">Preis</div>
        <div>
          <FormInputPrice class="w-full" name="price" required />
        </div>
      </div>
    </form>
  </Modal>
  ...
  methods: {
    ...
    upsert_ingredient() {
      ...
      call_ingredient_upsert(name, price).then(() => {
        this.$emit('upserted_ingredient', name, price);
